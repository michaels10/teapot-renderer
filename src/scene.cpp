...
void Scene::render(Canvas &canvas) {
    for (unsigned i = 0; i < canvas.width; i++) {
        for (unsigned j = 0; j < canvas.height; j++) {
            Ray ray = camera.get_initial_ray(i, j, canvas);
...
float Scene::render_ray(const Ray &ray, float multiplier, int reflection_count) {
    // Kick out if too many reflections or intensity is negligible
    ...
    // Kick out if there is no intersection.
    if (!hit.hit)
        return 0;

    float ret = local_illuminate(hit) * hit.scattering;
    // If this object does is neither transparent nor reflective, kick out
    if (hit.scattering + EPS >= 1)
        return ret;

    float fresnel_intensity = 1 - hit.scattering;
    float reflection_intensity = fresnel(ray, hit);
    Ray reflection_ray = reflect(ray, hit);
    ret += render_ray(reflection_ray, multiplier * fresnel_intensity * reflection_intensity, reflection_count + 1);
    if (reflection_intensity + EPS < 1.0) {
        float refraction_intensity = 1 - reflection_intensity;
        Ray refraction_ray = refract(ray, hit);
        ret += render_ray(refraction_ray, multiplier * refraction_intensity * fresnel_intensity, reflection_count + 1);
    }
    return ret;
}
